...
export const IncentivesModal: React.FC<IncentivesModalProps> = ({
  ...
  // Quote list is filtered by term + lease/finance; the deal card's list is a static snapshot
  const availableIncentives = quoteResult?.availableIncentives || deal?.availableIncentives || [];
  ...
  const totalSaved = useMemo(() => {
    return availableIncentives
      .filter((inc: any) => {
        const isFtbIncentive = inc.type === 'first_time_buyer' || inc.name?.toLowerCase().includes('first time buyer');
        return selectedIncentives.includes(inc.id) || autoApplied.some(a => a.id === inc.id) || (isFtbIncentive && isFirstTimeBuyer);
      })
        ...
        >
          ...
          <div className="w-full md:w-3/5 p-6 md:p-8 overflow-y-auto max-h-[80vh]">
            ...
            <div className="space-y-4">
              ...
              {/* Auto Selected (OEM Cash) — pre-checked, but the user can uncheck */}
              {autoSelected.map(inc => {
                const isSelected = selectedIncentives.includes(inc.id);
                return (
                <button
                  key={inc.id}
                  onClick={() => toggleIncentive(inc.id)}
                  className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all text-left ${
                    isSelected ? 'border-gray-100 bg-gray-50' : 'border-gray-200 bg-white hover:border-gray-300'
                  }`}
                >
                  <div className="flex items-center gap-3">
