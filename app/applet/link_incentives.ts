...
async function main() {
  console.log('Starting to link incentives to deals...');
  ...
  for (const deal of deals) {
    ...
    try {
      ...
      if (matchedIncentives.length > 0) {
        ...
        if (hasChanges) {
          // Add Hyundai OEM cash or general rebates into array
          data.availableIncentives = merged;
          
          await prisma.dealRecord.update({
            where: { id: deal.id },
            data: {
              financialData: JSON.stringify(data)
            }
          });
          updatedCount++;
        }
  ...
  console.log(`Successfully updated ${updatedCount} deals with incentives.`);
}
